...
export async function GET(request: NextRequest) {
  try {
    ...
    if (filters.callRoundId) {
      where.project = {
        callRoundId: filters.callRoundId,
      };
    }
    ...
    return NextResponse.json(
      { success: false, message: 'Lỗi khi lấy danh sách giải ngân' },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!['DEAN', 'ADMIN'].includes(actorRole)) {
      return NextResponse.json(
        { success: false, message: 'Bạn không có quyền tạo giải ngân' },
        { status: 403 }
      );
    }
    ...
    if (!project) {
      return NextResponse.json(
        { success: false, message: 'Không tìm thấy đề tài' },
        { status: 404 }
      );
    }
    ...
    // Kiểm tra ngân sách
    if (!project.budgetApproved || project.budgetApproved <= 0) {
      return NextResponse.json(
    ...
    return NextResponse.json(
      { success: false, message: 'Lỗi khi tạo giải ngân' },
      { status: 500 }
    );
  }
}
