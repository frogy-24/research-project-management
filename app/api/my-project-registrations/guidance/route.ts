...
export async function GET(req: Request) {
    try {
            ...
            new Set(
                registrations.flatMap((registration) => {
                    const members = Array.isArray(registration.teamMembers)
                        ? (registration.teamMembers as Array<{ studentId?: string }>)
                        : [];
                    return members
        ...
        const studentCodeById = new Map(memberUsers.map((user) => [user.id, user.code]));

        const hydratedRegistrations = registrations.map((registration) => {
            const members = Array.isArray(registration.teamMembers)
                ? (registration.teamMembers as Array<Record<string, unknown>>)
                : [];

            return {
                ...registration,
                teamMembers: members.map((member) => {
                    const studentId = typeof member.studentId === 'string' ? member.studentId : undefined;
                    return {
                        ...member,
                        studentCode: studentId ? (studentCodeById.get(studentId) ?? null) : null,
                    };
                }),
            };
        });
