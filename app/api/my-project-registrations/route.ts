...
import type { ApplicableFor, Prisma } from '@/prisma/generated/prisma';
...
const isAcceptedTeamMember = (rawMembers: unknown, userId: string): boolean => {
...
export async function POST(request: Request) {
    try {
        ...
        if (!canManageOwnRegistrations(actorRole)) {
            return NextResponse.json({ success: false, error: 'Bạn không có quyền đăng ký đề tài.' }, { status: 403 });
        }
        ...
        const applicableFor: ApplicableFor[] = actorRole === 'STUDENT' ? ['STUDENT', 'BOTH'] : ['LECTURER', 'BOTH'];
        ...
        const existingRegistrationInRound = await prisma.projectRegistration.findFirst({
            where: {
                ...
                OR: [
                    ...
                    {
                        AND: [{ status: 'PENDING' }, { instructorStatus: { not: 'REJECTED' } }],
                    },
                ],
            },
            select: { id: true },
        ...
        if (acceptedMembership) {
            return NextResponse.json(
                {
                    success: false,
                    error: 'Bạn đã tham gia một đề tài trong đợt này (đã xác nhận lời mời nhóm), không thể đăng ký thêm đề tài khác.',
                },
                { status: 400 },
            );
        }
        ...
        const selectedInstructor = activeCallRound.availableInstructors.find(
            (instructor) =>
                instructor.instructorId === parsed.data.instructorId && instructor.invitationStatus === 'ACCEPTED',
        );
        ...
        if (!selectedInstructor) {
            return NextResponse.json(
                {
                    success: false,
                    error: 'Giảng viên được chọn chưa chấp nhận lời mời tham gia đợt này hoặc không thuộc danh sách được mời.',
                },
                { status: 400 },
            );
        }
        ...
        if (inviteeIds.length > 0) {
            await createNotifications(
                inviteeIds.map((inviteeId) => ({
                    ...
                    message: `Bạn vừa được thêm vào nhóm của đề tài \"${created.title}\". Vui lòng xác nhận tham gia trong vòng 1 ngày, quá hạn lời mời sẽ tự động hủy.`,
