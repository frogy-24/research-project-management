      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Thống kê đề tài và giải ngân theo năm (phạm vi 10 năm)
          </p>
        </div>
              ...
              <CommandList>
                ...
                <CommandGroup>
                  {yearOptions.map(year => (
                    ...
                    >
                      <Check className={cn("mr-2 h-4 w-4", selectedYears.includes(year) ? "opacity-100" : "opacity-0")} />
                      {year}
                    </CommandItem>
                  ))}
                </CommandGroup>
...
function YearReportSection({
  ...
  const disbursementChartData = data.map((d: any) => ({
    year: d.year,
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND", maximumFractionDigits: 0 }).format(aggregated.totalDisbursed)}
            </div>
          </CardContent>
        </Card>
        <Card>
                  ...
                  <tbody className="divide-y">
                    {allRegistrations.map((r: any, idx: number) => (
                      <tr key={`${r.year}-${r.id}-${idx}`} className="hover:bg-muted/50">
                        ...
                        <td className="px-4 py-3 text-center">
                          {r.result ? <Badge>{r.result}</Badge> : <span className="text-muted-foreground">—</span>}
                        </td>
                  ...
                  <tbody className="divide-y">
                    {allDisbursements.map((d: any, idx: number) => (
                      <tr key={`${d.year}-${d.id}-${idx}`} className="hover:bg-muted/50">
                        ...
                        <td className="px-4 py-3 max-w-md truncate font-medium">{d.projectTitle || "—"}</td>
