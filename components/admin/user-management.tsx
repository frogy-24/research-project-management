...
export function UserManagement() {
  ...
  const filteredMajors = majorFilter === "ALL" && departmentFilter !== "ALL"
    ? majors.filter((m: any) => m.departmentId === departmentFilter)
    : majors;

  const filteredClasses = classFilter === "ALL" && majorFilter !== "ALL"
      ...
      <CardContent>
        ...
        {pagination && (
          <div className="mb-2 text-sm text-muted-foreground">
            Hiển thị {((currentPage - 1) * limit) + 1}–{Math.min(currentPage * limit, pagination.total)} / Tổng: <Badge variant="secondary">{pagination.total}</Badge> người dùng
          </div>
        )}
