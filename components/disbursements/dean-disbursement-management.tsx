          ...
          <DisbursementListTable
            ...
            onToggleSelect={(item, checked) => {
              setSelectedApproveIds((prev) =>
                checked ? (prev.includes(item.id) ? prev : [...prev, item.id]) : prev.filter((id) => id !== item.id)
              );
            }}
            ...
            <div className="space-y-5 text-sm">
              <div className="rounded-lg border p-4">
                ...
                <div className="font-medium">
                  {(selectedDisbursement.project?.code ?? 'N/A') + ' - ' + (selectedDisbursement.project?.title ?? '')}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                <div className="rounded-lg border p-3">
                  ...
                  <div className="mt-1 font-semibold text-base">{Number(selectedDisbursement.amount).toLocaleString('vi-VN')} VNĐ</div>
                </div>
                <div className="rounded-lg border p-3">
                  <div className="text-xs text-muted-foreground">Ngày giải ngân</div>
                  <div className="mt-1 font-medium">{new Date(selectedDisbursement.disbursedAt).toLocaleDateString('vi-VN')}</div>
                </div>
                <div className="rounded-lg border p-3">
                ...
                <div className="rounded-lg border p-3">
                  ...
                  <div className="mt-1"><DisbursementStatusBadge status={selectedDisbursement.status} /></div>
                </div>
                <div className="rounded-lg border p-3 md:col-span-2">
                  <div className="text-xs text-muted-foreground">Người tạo</div>
                  <div className="mt-1 font-medium">{selectedDisbursement.createdBy?.name ?? 'N/A'}</div>
                </div>
              </div>
                ...
                {selectedDisbursement.voucherFileUrl ? (
                  <div className="flex flex-wrap gap-2">
                    <Button asChild variant="outline" size="sm">
                      <a href={selectedDisbursement.voucherFileUrl} target="_blank" rel="noopener noreferrer">
              ...
              {selectedDisbursement.status === 'PENDING' ? (
                <div className="rounded-lg border p-4 space-y-3">
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    <Button
                      variant="destructive"
                      disabled={approveMutation.isPending || rejectMutation.isPending || !rejectionNote.trim()}
                      onClick={async () => {
